...
public class HTNDomainVisualizerVertical {
    public static void main(String args[]) throws Exception {
        //{
            DomainDefinition dd = DomainDefinition.fromLispFile("ahtn/microrts-ahtn-definition-low-level-phase-sensor.lisp");
            
        //    DomainDefinition dd = DomainDefinition.fromLispFile("ahtn/microrts-ahtn-definition-low-level_sensor.lisp");
        ...
        //}
        {
        	String pngname = "test1.png";//String pngname = "microrts-ahtn-definition-portfolio-phase"+".png";
        	//DomainDefinition dd = DomainDefinition.fromLispFile("ahtn/microrts-ahtn-definition-low-level-phase.lisp");
            //DomainDefinition dd = DomainDefinition.fromLispFile("ahtn/microrts-ahtn-definition-portfolio-phase.lisp");
            HTNDomainVisualizerVertical v = new HTNDomainVisualizerVertical();
            BufferedImage img = v.visualizeHTNDomain(dd, new Symbol("destroy-player"));
            File pfile = new File(pngname); 
            int width = img.getWidth(null);
            int height = img.getHeight(null);
            BufferedImage bufimag = new BufferedImage(width,height,BufferedImage.TYPE_INT_RGB);
            Graphics2D g2d = bufimag.createGraphics();
            g2d.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_ATOP, 0.0f));
            g2d= bufimag.createGraphics();           
            ...
            ImageIO.write(bufimag, "png", pfile);
        }
        {
    ...
    public BufferedImage visualizeHTNTask(DomainDefinition d, Symbol root, List<Symbol> alreadyProcessed) throws Exception {
        if (alreadyProcessed.contains(root)) {
           ...
           // System.out.println("  visualizeHTNTask img: " + img.getWidth() + " x " + img.getHeight() + " (methodHeight: " + methodHeight + ")");  
            ...
            g2d.setColor(Color.BLACK);
            g2d.drawRect(0,height/2 - (fm.getHeight()+2*vpadding)/2, textWidth+2*hpadding, fm.getHeight() + vpadding*2);
            g2d.setColor(Color.BLACK);
    ...
    //MK
    public BufferedImage visualizeHTNPhase(DomainDefinition d, HTNMethod m, List<Symbol> alreadyProcessed) throws Exception {
    ...
    public BufferedImage visualizeHTNMethod(DomainDefinition d, HTNMethod m, List<Symbol> alreadyProcessed) throws Exception {
       ...
       // System.out.println("visualizeHTNMethod: " + m.getName());        
        ...
        for(Symbol c:children) {
            ...
            //MK
            else if(c.get().startsWith("phase"))
            {
            	for(HTNMethod mphase:d.getPhases())
            	{
            		if(c.get().matches(mphase.getName())!=true)
            			continue;
            		BufferedImage img2 = visualizeHTNPhase(d,mphase,alreadyProcessed);
                    images.add(img2);
                    childrenWidth = Math.max(childrenWidth, img2.getWidth());
                    childrenHeight += img2.getHeight();
            	}

...
public class AdversarialPlan {
    ...
    // depth is in actions:
    public AdversarialPlan(Term goalPlayerMax, Term goalPlayerMin, int a_maxPlayer, int depth, int playoutLookahead, GameState a_gs, DomainDefinition a_dd, DomainDefinition a_ddenemy, EvaluationFunction a_f, AI a_playoutAI) {
        HTNMethod max = null;
        HTNMethod min = null;
...
//        System.out.println(a_dd);
    ...
    public Pair<MethodDecomposition,MethodDecomposition> getBestPlan(long timeLimit, int maxPlayouts, boolean forceAnswer) throws Exception 
    {
        ...
        do
        {
            ...
            boolean pop = true;
            while(!timeout && choicePoint.nextExpansionTest(dd,ddenemy, bindings, renamingIndex, choicePoint)) {
                renamingIndex++;
...
//                    System.out.println("trail.add(" + (bindings.size() - bl) + ")");
    ...
    */
    public static Pair<MethodDecomposition,MethodDecomposition> getBestPlanIterativeDeepening(Term goalPlayerMax, Term goalPlayerMin, int a_maxPlayer, int timeout, int maxPlayouts, int a_playoutLookahead, GameState a_gs, DomainDefinition a_dd, DomainDefinition a_ddenemy,EvaluationFunction a_f, AI a_playoutAI,boolean flag) throws Exception 
    {
        ...
        for(int depth = 1;;depth++)
        {
            ...
            AdversarialPlan planner = new AdversarialPlan(goalPlayerMax, goalPlayerMin, a_maxPlayer, depth, a_playoutLookahead, a_gs, a_dd, a_ddenemy,a_f, a_playoutAI);
            //planner是一个规划器
...
//                     System.out.println("testing non-branching condition: " + cp_md);
                     if (!acp.nextExpansionTest(dd, ddenemy,bindings, renamingIndex, acp)) 
                     {
                         return null;
                     }
                     renamingIndex++;

...
public class DomainDefinition {
    ...
    //MK
    List<HTNPhase> phases = new LinkedList<>();  //每个方法下面的所属的阶段
        ...
        for(LispElement def_e:rest_e.children) {
            ...
            if (def_e.children!=null && def_e.children.size()>0) {         	
                ...
                if (head.element!=null && head.element.equals(":operator")) {
                    ...
                    HTNMethod m = HTNMethod.fromLispElement(def_e);  //保存的是对于method的分解，包括条件，顺序执行、各个阶段，暂时兼容旧的格式
                    dd.addMethod(m);
                } 
                //MK
                else if(head.element!=null && head.element.equals(":phase"))
                {
                	 HTNPhase m = HTNPhase.fromLispElement(def_e); //保存对于phase的分解，包括执行条件、并行执行、子任务
                     dd.addPhase(m);
                }
                //
                else {

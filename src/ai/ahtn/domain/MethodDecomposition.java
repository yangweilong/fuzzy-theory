...
public class MethodDecomposition {
    ...
    //MK                                                            // and won't result in a choicepoint
    public static final int METHOD_PHASE = 7;
    
    public static final int METHOD_sufficient = 8;
    public static final int METHOD_necessary = 9;
    
    HTNPhase phase = null;
    public HTNPhase operatorownedphase = null;
    ...
    public MethodDecomposition[] subelements = null;
    ...
    */
    ...
    //MK
    public MethodDecomposition(Term t, HTNPhase m) {
        type = METHOD_PHASE;
        term = t;
        phase = m;
    }
    ...
    public void setPhase(HTNPhase m) {
        phase = m;
    }
    ...
    public static MethodDecomposition fromLispElement(LispElement e) throws Exception {
        ...
        //MK
        else if (head.element.equals(":phase"))
        {
        	MethodDecomposition d = new MethodDecomposition();
        	d.type = METHOD_PHASE;
        	d.term = Term.fromLispElement(e.children.get(1));
        ...
        else if(head.element.equals(":sufficient"))
        {
        	MethodDecomposition d = new MethodDecomposition();
        	d.type = METHOD_sufficient;
        	d.clause = Clause.fromLispElement(e.children.get(1));
        	return d;
        }
        else if(head.element.equals(":necessary"))
        {
        	MethodDecomposition d = new MethodDecomposition();
        	d.type = METHOD_necessary;
        	d.clause = Clause.fromLispElement(e.children.get(1));
        	return d;
        }
        //
    ...
    public void printDetailed(int tabs) {
        ...
        switch(type) {
            ...
            case METHOD_PHASE:
                if (phase!=null) {
                    System.out.println(this.hashCode() + " - "+executionState+" - (:phase " + phase.head + ")");                    
                    for(int j = 0;j<tabs;j++) System.out.print("  ");
                    System.out.println("Decomposition:");
                    phase.getDecomposition().printDetailed(tabs+1);
                } else {
                    System.out.println(this.hashCode() + " - "+executionState+" - (:phase " + term + ")");                    
                }
                break;
                //
            case METHOD_SEQUENCE:
    ...
    */
    public int executionCycle(GameState gs, List<MethodDecomposition> actions, List<MethodDecomposition> choicePoints, AdversarialChoicePoint previous_cp,List<Binding> bindings) {
        switch(type) {
            ...
            case METHOD_METHOD:
             ...
             case METHOD_sufficient:
             case METHOD_necessary:
             {
            	 if (executionState==0) {
                     choicePoints.add(this);
                     return EXECUTION_CHOICE_POINT;
            ...
            case METHOD_SEQUENCE:
                do {
                    if (executionState>=subelements.length) 
                        return EXECUTION_SUCCESS;
                    int tmp = subelements[executionState].executionCycle(gs, actions, choicePoints, previous_cp,bindings);
                    if (tmp==EXECUTION_SUCCESS) {
                        previous_cp.captureExecutionStateNonRecursive(this);
                        executionState++;
                    } else {
            ...
            case METHOD_PARALLEL:
                {
                    ...
                    for(int i = 0;i<subelements.length;i++) 
                    {
                        int tmp = subelements[i].executionCycle(gs, actions, choicePoints, previous_cp,bindings);
                        if (tmp==EXECUTION_ACTION_ISSUE) anyActionIssue = true;
    ...
    public void convertToOperatorList(List<Pair<Integer,List<Term>>> l) throws Exception {
        switch(type) {
            ...
            case METHOD_PHASE:
            {
            	if (phase!=null) phase.getDecomposition().convertToOperatorList(l);
                break;
            }
             //
        }
    ...
    public boolean ProccessConditions(int player, GameState gs, MethodDecomposition toExcute) throws Exception {
    	...
    	HTNMethod method =toExcute.getMethod();
    	MethodDecomposition[] submint;
    	if(method==null)
		{
    		submint= toExcute.getSubparts();
		}
    	else
    	{
    		submint = method.getDecomposition().getSubparts();
    	}
    	if(submint==null)
    	{
    		return true;
...
//			   }
			   ...
			   {
				   if(!submint[i].getPhase().ProccessConditions(player,gs,submint[i]))
					   return false;
				   return true;
			   }
			}

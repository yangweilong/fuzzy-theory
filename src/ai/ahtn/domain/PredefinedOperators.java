...
//                                    anyunit = true;
                            ...
                            if (pa==null) {
                                ...
                                for(Unit u:gs.getUnits()) {
                                    if (u.getPlayer()==player) {
                                        ...
                                        {
                                        	//MK
                                        	 UnitAction aa = new UnitAction(UnitAction.TYPE_NONE, 10,state.operatorownedphase);
                                        	if(((IntegerConstant)t.parameters[1]).value==0)
                                        		aa.Essential = false;
                                        	else
                                        		aa.Essential = true;
                                        	aa.planlist = state.planlist;
                                            pa.addUnitAction(u,aa);
                                            //
            ...
            operators.put(new Symbol("!sensor"),
                    new OperatorExecutor() 
            {
                        public boolean execute(Term t, MethodDecomposition state, GameState gs, PlayerAction pa) throws Exception 
                        {
                            ...
                            else
                            {
                                if (pa==null) 
                                {
                                    pa = new PlayerAction();
                                    //MK
                                    UnitAction aa = new UnitAction(UnitAction.TYPE_SENSOR, 10,state.operatorownedphase);
                                    if (((IntegerConstant)t.parameters[1]).value==0)
                                    	aa.Essential = false;
                                    else
                                    	aa.Essential = true;
                        ...
                        public boolean execute(Term t, MethodDecomposition state, GameState gs, PlayerAction pa) throws Exception {
                            ...
                            if (state.getOperatorExecutingState()==1) {
                                ...
                                if (pa==null) {
                                    ...
                                    UnitAction aa = new UnitAction(UnitAction.TYPE_ATTACK_LOCATION,u2.getX(),u2.getY(),state.operatorownedphase);
                                                                      
                                    if (((IntegerConstant)t.parameters[2]).value==0)
                                    	aa.Essential = false;
                                    else
                                    	aa.Essential = true;
                                    aa.planlist = state.planlist;
                        ...
                        public boolean execute(Term t, MethodDecomposition state, GameState gs, PlayerAction pa) throws Exception {
                            ...
                            if (state.getOperatorExecutingState()==1) {
                                ...
                                if (pa==null) {
                                    ...
                                    if (u1.getX() == u2.getX()-1) pa.addUnitAction(u1, new UnitAction(UnitAction.TYPE_HARVEST, UnitAction.DIRECTION_RIGHT,state.operatorownedphase));
                                    if (u1.getX() == u2.getX()+1) pa.addUnitAction(u1, new UnitAction(UnitAction.TYPE_HARVEST, UnitAction.DIRECTION_LEFT,state.operatorownedphase));
                                    if (u1.getY() == u2.getY()-1) pa.addUnitAction(u1, new UnitAction(UnitAction.TYPE_HARVEST, UnitAction.DIRECTION_DOWN,state.operatorownedphase));
                                    if (u1.getY() == u2.getY()+1) pa.addUnitAction(u1, new UnitAction(UnitAction.TYPE_HARVEST, UnitAction.DIRECTION_UP,state.operatorownedphase));
                                    gs.issue(pa);
                                } else {
                                    if (u1.getX() == u2.getX()-1) pa.addUnitAction(u1, new UnitAction(UnitAction.TYPE_HARVEST, UnitAction.DIRECTION_RIGHT,state.operatorownedphase));
                                    if (u1.getX() == u2.getX()+1) pa.addUnitAction(u1, new UnitAction(UnitAction.TYPE_HARVEST, UnitAction.DIRECTION_LEFT,state.operatorownedphase));
                                ...
                                //MK
                                if(pa.getAction(u1)!=null)
                                {
                                	 if(((IntegerConstant)t.parameters[2]).value==0)
                                         pa.getAction(u1).Essential = false;
                                     else
                                     	pa.getAction(u1).Essential = true;
                                	 pa.getAction(u1).planlist = state.planlist;
                                }
                                //
                                state.setOperatorExecutingState(1);
                                return false;
                        ...
                        public boolean execute(Term t, MethodDecomposition state, GameState gs, PlayerAction pa) throws Exception {
                            ...
                            if (state.getOperatorExecutingState()==1) {
                                ...
                                if (pa==null) {
                                    ...
                                    if (u1.getX() == u2.getX()-1) pa.addUnitAction(u1, new UnitAction(UnitAction.TYPE_RETURN, UnitAction.DIRECTION_RIGHT,state.operatorownedphase));
                                    if (u1.getX() == u2.getX()+1) pa.addUnitAction(u1, new UnitAction(UnitAction.TYPE_RETURN, UnitAction.DIRECTION_LEFT,state.operatorownedphase));
                                    if (u1.getY() == u2.getY()-1) pa.addUnitAction(u1, new UnitAction(UnitAction.TYPE_RETURN, UnitAction.DIRECTION_DOWN,state.operatorownedphase));
                                    if (u1.getY() == u2.getY()+1) pa.addUnitAction(u1, new UnitAction(UnitAction.TYPE_RETURN, UnitAction.DIRECTION_UP,state.operatorownedphase));
                                    gs.issue(pa);
                                } else {
                                    if (u1.getX() == u2.getX()-1) pa.addUnitAction(u1, new UnitAction(UnitAction.TYPE_RETURN, UnitAction.DIRECTION_RIGHT,state.operatorownedphase));
                                    if (u1.getX() == u2.getX()+1) pa.addUnitAction(u1, new UnitAction(UnitAction.TYPE_RETURN, UnitAction.DIRECTION_LEFT,state.operatorownedphase));
...
//                                System.out.println("PRODUCE working fine for "  + uID1 + "!");
                                    ...
                                    gs.getPlayer(u1.getPlayer()).getResources()-ru.getResourcesUsed(u1.getPlayer())>=ut.cost) 
                                {
                                    if (pa==null) 
                                    {
                                        pa = new PlayerAction();
                                        //MK
                                        UnitAction aa = new UnitAction(UnitAction.TYPE_PRODUCE, direction, ut,state.operatorownedphase);
                                        aa.planlist = state.planlist;
                                        //MK
                                         aa.ownedphase =state.operatorownedphase;
                                        //                                         
                                        if (((IntegerConstant)t.parameters[3]).value==0)
                                                aa.Essential = false;
                                        else
                                        	aa.Essential = true;
                                        pa.addUnitAction(u1, aa);
                                    ...
                                    {
                                        UnitAction ua = new UnitAction(UnitAction.TYPE_PRODUCE, direction, ut,state.operatorownedphase);
                                        ResourceUsage ru2 = ua.resourceUsage(u1, gs.getPhysicalGameState());
                                        pa.getResourceUsage().merge(ru2);
                                        //MK
                                        ua.ownedphase =state.operatorownedphase;
                                        ua.planlist = state.planlist;
                                       //
                                        //MK
                                        if (((IntegerConstant)t.parameters[3]).value==0)
                                            ua.Essential = false;

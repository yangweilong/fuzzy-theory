...
public class UnitAction {
    ...
    public static final int TYPE_SENSOR = 6;//YANG感知
//    public static final int TYPE_SENSOR_OPEN  = 7;
    public static final int NUMBER_OF_ACTION_TYPES = 8;
    public static String actionName[] ={"none","move","harvest","return","produce","attack_location","sensor"};
 //   public static String actionName[] ={"none","move","harvest","return","produce","attack_location"};
    ...
    public int status = 1;     //0，1 表示失败、成功
    public boolean canexecuteble = true; //是否暂时失败
    public HTNPhase ownedphase = new HTNPhase("",null,null);
    public MethodDecomposition Topplan = null;
    public List<UnitAction> causelink = null;
    public List<AdversarialChoicePoint> planlist = new LinkedList();
    
    public UnitAction(int a_type, HTNPhase phase) {
        type = a_type;
        ownedphase = phase;
        causelink = new LinkedList<>();
    }
    
    public UnitAction(int a_type, int a_direction, HTNPhase phase) {
        type = a_type;
    ...
    public UnitAction(int a_type, int a_direction, UnitType a_unit_type, HTNPhase phase) {
        type = a_type;
        parameter = a_direction;
    ...
    public UnitAction(int a_type, int a_x, int a_y, HTNPhase phase) {
        type = a_type;
        x = a_x;
        y = a_y;
        ownedphase = phase;
...
//    public UnitAction(int a_type,Boolean on_off, int a_direction,HTNPhase phase) {
//        type = a_type;
//        onoff = on_off;
...
//            break;
    ...
    public int ETA(Unit u) {
        switch(type) {
            ...
            case TYPE_SENSOR:
            	return u.getSensorTime();//yang感知
//            case TYPE_SENSOR_OPEN:
//            	return u.getSensoropenTime();//yang感知
        }
            ...
            case TYPE_RETURN:
                {
                    ...
                    p.settotalResources(p.getResources()+1);//yang
                    u.setResources(0);
                }
                break;
            case TYPE_PRODUCE:
...
//	            {
////	            	Player p = pgs.getPlayer(u.getPlayer());
//	            	u.setSightRadius(u.getType().sightRadius+4);
////	            	p.setResources(p.getResources() - sensor_Cost);
//	            }
//	            break;
            case TYPE_SENSOR:
	            {	            		       
